  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-3xl font-heading font-bold tracking-tight text-text-primary">Job Board</h1>
        <p className="text-text-secondary font-body mt-1">Discover and apply for your next big opportunity</p>
      </motion.div>
      ...
      {jobs.length === 0 ? (
        <GlassCard className="text-center p-12">
          ...
          <h3 className="text-xl font-heading font-semibold text-text-primary">No Jobs Posted Yet</h3>
              ...
              <GlassCard>
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                  <div className="flex-1">
                    <h3 className="font-heading font-semibold text-text-primary text-xl mb-1">{job.title}</h3>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {job.requirements.map((req, i) => (
                        <Badge key={i} variant="default">{req}</Badge>
                      ))}
                    </div>
                  </div>
                  <div className="flex md:flex-col items-center gap-3 w-full md:w-auto">
                    {appliedJobs[job.id] ? (
                      <div className="flex items-center gap-2 text-lime font-medium px-4 py-2 border border-lime/30 rounded-lg bg-lime/10 w-full justify-center">
